...
export default function CarteiraPro() {
  ...
  // Contador para IDs únicos (evita Date.now() no SSR)
  ...
  const sendMessage = async () => {
    if (!inputMessage.trim() || isLoadingChat || !isMounted) return
  ...
  const addStockToPortfolio = () => {
    ...
    if (existingStock) {
      ...
      // Usa contador incremental em vez de Date.now()
      idCounterRef.current += 1
      setPortfolioStocks([...portfolioStocks, {
        id: `stock-${idCounterRef.current}`,
        ticker,
        quantity
      }])
    }
  ...
  const addExpense = () => {
    ...
    setExpenses([...expenses, {
      id: `expense-${idCounterRef.current}`,
      name,
      value,
      type: newExpenseType
    }])
